...
**Roster rows (V1) — ⚠ under active re-probe (todo.md R1–R5); the model below is provisional.** Direction (2026-07-04): the row list unions *everyone present at the kill* with *everyone currently in the raid* (latecomers marked **ineligible**); "more data is better." Today `session.rows` seeds empty and fills only from `cResp`. New: at loot time the ML snapshots the **present roster** onto each captured item (attach in `OnChatMsgLoot`, `Award.lua:177-189`, carried through `BuildCouncilableList` → `sessionItems`/staging alongside `boss/instance/lootedAt`). At `StartSession`, `session.rows[i]` is **pre-seeded** one row per captured-present raider via `PresentRoster()` + `ClassCanUse(link, class)`:
```lua
rows[i][name] = { name, class, resp=nil, reason="pending"|"cantuse"|"left", votes=0, note, gear }
```
eligible+present → `pending`; can't use → `cantuse` (auto-pass, a non-roll); in snapshot but no longer present → `left`. An incoming `cResp` merges into the seeded row (the `prev = rows[key]` path, `Session.lua:314`), setting `resp` and clearing `reason`; **the seed's `class`/`reason` must survive the overwrite** (`Session.lua:315-321` preserves only `votes` today). Row shape extends to `{ name, class, resp, reason, note, gear, votes }`.

**Caveat — "present at the kill" ≈ "present when looted."** No boss-kill event is observed (DL-7 auto-loot; `boss` is the ML's `UnitName("target")` at the loot line), so the snapshot is taken at **loot time** (a close proxy). Manually-added items (no loot context) fall back to the **present-at-`StartSession`** roster. A real `ENCOUNTER_END`/`UNIT_DIED` kill hook is a possible later refinement (DL-15).

**Display (V1).** Rollers (non-PASS responders) sort on top by response/votes; **all non-rollers (pending / pass / can't-use / left) sink to the bottom, dimmed** (RCLC). The item rail orders **chronologically, oldest loot first** (`lootedAt`; manual-adds last).

**Readiness border (V3/V4).** The **ML computes a per-item `status` and broadcasts it** (new `cUpdate.status`) so every client draws the same **rail-row** border (header icon unchanged — V4), applied receiver-side like `awarded` (the `award`-flow template) and painted in `FillLootRailRow` (`LootWindow.lua:208-243`). Present-eligible = present raiders who can use the item; "wants it" = a non-PASS response; "voted" = ≥1 non-zero vote (no abstain — V2):
...
- **DL-15 (accepted, 2026-07-04 — Feature V voting readiness):** `session.rows` is pre-seeded one row per present raider (`PresentRoster` + `ClassCanUse`); non-rollers dim to the bottom (RCLC); the rail sorts oldest-loot-first. The ML computes a per-item **status** (awarded/de/ready/voting/waiting) and broadcasts it on `cUpdate` so all clients draw the same **rail-row** border (header unchanged). Adds a vote tally (`status.voted={n,of}`), anonymous voting (snapshotted onto `sStart.anon`, default off), and a **D/E award type** (`STATUS.DISENCHANT=93`; target = highest-ranked present+eligible `config.disenchanters`; reason renders "D/E"). **Caveat:** "present at kill" is approximated by "present when looted" (no kill event; DL-7 auto-loot) — manual-adds fall back to present-at-`StartSession`; a real `ENCOUNTER_END`/`UNIT_DIED` hook is a later refinement. **Update (2026-07-04): the exact row-set / eligibility model is being re-probed** — direction: union everyone-present-at-kill with everyone-currently-in-raid (latecomers = ineligible), "more data is better"; see todo.md R1–R5. §6.10.
