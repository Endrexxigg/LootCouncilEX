...
Adopts the CLA "gear issues" model (`docs/CLA_gear_issues_findings.md`) as a **viewer-side** analysis over the gear links already in `gearCache` (§6.3) — **no comms or protocol change** (DL-13). Rules ship as static data (`Data/GearRules.lua`); `Core/GearIssues.lua` is the pure, headless-tested evaluator; results surface in the Roster → Gear sub-tab (per-item tags) and a **Gear Check** overview *within* the Roster tab (the Players module renamed to **Roster** — everyone + issue counts, the pre-raid slacker scan). **Display-only in v1** (no auto-whisper). v1 shipped the **core three** checks (enchant / empty-socket / gem-quality); **v1.1 (Phase 17)** adds three more kinds, all fail-open: **`useless`** (`GearRules.itemCondition[itemID]` → a labeled FYI tag, e.g. `[PvP trinket]`), **`metagem`** (`GearMetaIssue` — the equipped meta's colour requirement vs the whole set's gem colours, once per player, skipped if the meta or any gem is unknown), and **`socketcolor`** (`ItemSocketColors` per-colour counts + a matching-feasibility check — flag `[socket bonus unmet]` only when every socket is filled, every gem colour is known, and no assignment satisfies the socket colours; hybrids match both parents). The colour/requirement/condition tables ship minimal; the evaluators are fixture-proven, the content fills in a pass. `IssueTagSuffix`/`BuildGearCheckDisplay` render the new kinds generically (no UI change).
...
**Phase 17 — Gear-issue v1.1 (DL-13 fast-follow).** The three deferred §6.8 checks, all **fail-open**
and viewer-side (no comms): **useless-item** (`GearRules.itemCondition[itemID]` → a labeled FYI tag,
e.g. `[PvP trinket]`/`[vs undead only]`; per-encounter flag tables ship for a future boss-aware
surface), **meta-gem activation** (`GearMetaIssue` evaluates the equipped meta's colour requirement
against the whole set's gem colours ONCE per player — skipped if the meta or any gem is unknown),
and **socket-colour matching** (`ItemSocketColors` per-colour counts + a feasibility check: flag
`[socket bonus unmet]` only when every socket is filled, every gem's colour is known, and NO
assignment satisfies the socket colours; hybrids match both parents). The colour/requirement/
condition tables ship minimal — the evaluators are proven by fixtures, the content fills in a pass.
*Exit:* a character with a flagged conditional item + an inactive meta shows both tags in Gear Check.
...
- **DL-13 (accepted, 2026-07-04 — Feature G gear issues):** enchant/gem issue detection is **viewer-side** over the links already in `gearCache` + `GetItemStats` sockets — **no comms/protocol change**. The one exception: if `GetItemStats` sockets prove unreliable on Anniversary (verify via selftest, X3), a reporter-side socket count is added to `pReport`/`gearCache`. Rules ship as **static `Data/GearRules.lua`** (CLA-derived: per-slot enchant **allowlist** that fails safe on unknown enchants, gem-min-quality = rare, excluded-gear whitelist), **not** guild-editable in v1. The boss-conditional "useless item" (undead/demon/PvP/engi) + meta-gem-activation checks, and true socket-color matching, were **deferred to a fast-follow — SHIPPED in Phase 17 (v1.1)**, all fail-open and still viewer-side (new kinds `useless`/`metagem`/`socketcolor`; the colour/requirement/condition data ships minimal and fills in a content pass). Model: `docs/CLA_gear_issues_findings.md`; canonical spec §6.8.
